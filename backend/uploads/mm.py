import random,os,smtplib,string,shutil,certifi
...
ca = certifi.where()
client=MongoClient(os.getenv("connectdb"), tlsCAFile=ca)
...
if "logdata" in dblist:
    db=client["logdata"]
    print(f"Collections in 'logdata' database: {db.list_collection_names()}")
